<template>
  <section v-if="vehicle" class="vehicle-page">
    ...
    <VehicleTabs v-model="activeTab" :tabs="tabs">
      ...
      <template #tab-ownership><VehicleSection title="Ownership data" eyebrow="Costs" description="Track purchase, legal dates, insurance, value, and running costs."><form class="edit-form" @submit.prevent="saveOwnership"><div class="edit-grid"><label>Purchase date<input v-model="ownershipDraft.purchaseDate" type="date" /></label><label>Purchase price<input v-model.number="ownershipDraft.purchasePrice" type="number" min="0" /></label><label>Insurance<input v-model="ownershipDraft.insurance" /></label><label>Inspection expiry<input v-model="ownershipDraft.inspectionExpiry" type="date" /></label><label>Registration<input v-model="ownershipDraft.registration" /></label><label>Current value<input v-model.number="ownershipDraft.currentValue" type="number" min="0" /></label><label>Fuel costs<input v-model.number="ownershipDraft.fuelCosts" type="number" min="0" /></label><label>Base ownership cost<input v-model.number="ownershipDraft.ownershipCost" type="number" min="0" /></label><label>Service history costs<output>{{ formatCurrency(serviceHistoryCost) }}</output></label><label>Total ownership cost<output>{{ formatCurrency(totalOwnershipCost) }}</output></label></div><div class="edit-actions"><button class="button button-primary" type="submit">{{ t('vehicle.save') }}</button></div></form></VehicleSection></template>

      <template #tab-modifications><VehicleSection title="Modifications" eyebrow="Changes" description="Record installed parts and keep compatibility context."><div class="section-toolbar"><span>{{ modifications.length }} modifications</span><button class="button button-primary" type="button" @click="showModificationForm = !showModificationForm">{{ showModificationForm ? 'Close' : 'Add modification' }}</button></div><form v-if="showModificationForm" class="edit-form nested-form" @submit.prevent="addModification"><div class="edit-grid"><label>Part installed<input v-model="modificationDraft.partInstalled" required /></label><label>Date<input v-model="modificationDraft.date" type="date" required /></label><label>Supplier<input v-model="modificationDraft.supplier" /></label><label>Part number<input v-model="modificationDraft.partNumber" /></label><label>Reason<input v-model="modificationDraft.reason" /></label><label>Compatibility notes<input v-model="modificationDraft.compatibilityNotes" /></label><label class="checkbox-field"><input v-model="modificationDraft.originalPartRetained" type="checkbox" /> Original part retained</label></div><div class="edit-actions"><button class="button button-primary" type="submit">Save modification</button></div></form><div v-if="modifications.length" class="record-grid"><article v-for="item in modifications" :key="item.id" class="record-card"><strong>{{ item.partInstalled }}</strong><span>{{ formatDate(item.date) }}<template v-if="item.partNumber"> · {{ item.partNumber }}</template></span><p v-if="item.supplier">Supplier: {{ item.supplier }}</p><small>{{ item.originalPartRetained ? 'Original part retained' : 'Original part not retained' }}</small></article></div><p v-else class="empty-copy">No modifications yet.</p></VehicleSection></template>
    </VehicleTabs>
  </section>
  <section v-else class="empty-state"><span class="eyebrow">{{ t('nav.garage') }}</span><h2>{{ t('vehicle.notFound') }}</h2><RouterLink to="/garage">{{ t('vehicle.return') }}</RouterLink></section>
</template>
...
import { useMaintenanceStore, type MaintenanceCategoryName } from '../stores/maintenance'
import type { FuelType, VehicleDiagnosis, VehicleModification, VehicleTechnicalReference } from '../models/vehicle'
...
const diagnosisCollisionRects = computed(() => ({
  ...
  causes: [diagnosisPanels.symptoms, diagnosisPanels.outcomes],
  outcomes: [diagnosisPanels.symptoms, diagnosisPanels.causes],
}))
const activeTab = ref('identity'); const showMaintenanceForm = ref(false); const showModificationForm = ref(false); const selectedDiagnosis = ref<VehicleDiagnosis | null>(null); const showDiagnosisEditor = ref(false)
const tabs = [{ value: 'identity', label: 'Vehicle identity' }, { value: 'service', label: 'Service history' }, { value: 'diagnosis', label: 'Problems and diagnosis' }, { value: 'technical', label: 'Technical reference' }, { value: 'ownership', label: 'Ownership data' }, { value: 'modifications', label: 'Modifications' }]
...
function openDiagnosis(item: VehicleDiagnosis) { selectedDiagnosis.value = item; showDiagnosisEditor.value = true }
function saveDiagnosis(item: VehicleDiagnosis) { if (!vehicle.value) return; const next = selectedDiagnosis.value ? diagnoses.value.map((existing) => existing.id === item.id ? item : existing) : [item, ...diagnoses.value]; vehicleStore.updateVehicle(vehicle.value.id, { diagnoses: next }); showDiagnosisEditor.value = false; selectedDiagnosis.value = null }
function deleteDiagnosis() { if (!vehicle.value || !selectedDiagnosis.value) return; vehicleStore.updateVehicle(vehicle.value.id, { diagnoses: diagnoses.value.filter((item) => item.id !== selectedDiagnosis.value?.id) }); showDiagnosisEditor.value = false; selectedDiagnosis.value = null }
function addModification() { if (!vehicle.value || !modificationDraft.partInstalled) return; const item: VehicleModification = { ...modificationDraft, id: makeId('modification') }; vehicleStore.updateVehicle(vehicle.value.id, { modifications: [item, ...modifications.value] }); modificationDraft.partInstalled = ''; modificationDraft.supplier = ''; modificationDraft.partNumber = ''; modificationDraft.reason = ''; modificationDraft.compatibilityNotes = ''; showModificationForm.value = false }
</script>
...
<style scoped>
.diagnosis-board { position: relative; min-width: 852px; min-height: 332px; margin: 22px 0; overflow: auto; border: 1px dashed var(--line); border-radius: 8px; background: var(--surface-muted); background-image: linear-gradient(to right, rgb(36 107 69 / 8%) 1px, transparent 1px), linear-gradient(to bottom, rgb(36 107 69 / 8%) 1px, transparent 1px); background-size: 16px 16px; }
.diagnosis-list { display: grid; gap: 10px; list-style: none; margin: 0; padding: 0; }
.diagnosis-list li { display: grid; gap: 3px; padding: 9px; border: 1px solid var(--line); border-radius: 6px; background: var(--surface); }
.diagnosis-list span { color: var(--muted); font-size: 11px; }
.record-edit { display: grid; width: 100%; gap: 6px; padding: 16px; border: 0; background: transparent; color: inherit; text-align: left; cursor: pointer; }.record-edit:hover { background: var(--surface-muted); }
@media (max-width: 680px) { .detail-heading { margin: 24px 0 18px; gap: 16px; }.detail-heading h2 { font-size: 28px; letter-spacing: -.8px; }.heading-actions, .heading-actions .button { width: 100%; }.heading-actions .button { min-height: 44px; text-align: center; }.hero-copy { padding: 20px; }.hero-copy strong { font-size: 30px; }.detail-stats { gap: 8px; margin-bottom: 28px; }.detail-stats article { padding: 14px; }.detail-stats strong { font-size: 18px; }.section-toolbar { width: 100%; gap: 12px; }.section-toolbar .button { width: 100%; min-height: 44px; }.diagnosis-board { min-width: 700px; min-height: 340px; margin: 16px 0; }.record-card { padding: 0; }.record-edit { min-height: 78px; padding: 14px; } }
@media (max-width: 680px) { .diagnosis-board { min-width: 0; min-height: 0; display: grid; gap: 12px; padding: 12px; overflow: visible; }.diagnosis-board :deep(.vehicle-movable-panel) { position: static; width: auto !important; height: auto !important; min-height: 220px; }.diagnosis-board :deep(.vehicle-movable-panel-hint) { display: none; } }
</style>
